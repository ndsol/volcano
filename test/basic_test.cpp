...
int mainLoop(GLFWwindow* window, SimplePipeline& simple) {
  ...
  command::Semaphore imageAvailableSemaphore(simple.cpool.vk.dev);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (next_image_i != (uint32_t)-1) {
      ...
      if (simple.cmdBuffers.at(next_image_i).enqueue(lock, info) ||
  ...
  VkResult v = vkDeviceWaitIdle(simple.cpool.vk.dev.dev);
  if (v != VK_SUCCESS) {
