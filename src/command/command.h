...
typedef struct PipelineStage {
  ...
  void specialize(std::vector<VkSpecializationMapEntry> map,
    ...
    specialization.resize(info.dataSize);
    memcpy(specialization.data(), info.pData, specialization.size());
  }
...
typedef struct PipelineCreateInfo {
  ...
  // stages contains a PipelineStage for each fixed-function stage on the GPU.
  ...
  // stageBits and entryPointName, but science::ShaderLibrary adds spirv_cross
  // reflection to reduce the boilerplate needed.
...
typedef struct Pipeline {
  // Pipeline constructor calls PipelineCreateInfo constructor with pass_, but
  // does not retain it. A Pipeline can be used for more than one RenderPass.
  ...
  // creates a VkPipeline. The RenderPass used is info.pass. This Pipeline's
  // index in it is passed in as a parameter. RenderPass::ctorError() calls
  // this method for you on all subpasses defined in it. But if your app uses
  // pipelines that are not in RenderPass (for changing state or whatever),
  // you will need to call this on the non-default pipelines.
  WARN_UNUSED_RESULT virtual int ctorError(RenderPass& pass, size_t subpass_i);
