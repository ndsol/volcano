...
int RenderPass::getVkRenderPassCreateInfo(
  ...
  for (size_t i = 0; i < pipelines.size(); i++) {
    ...
    VkSubpassDescription VkInit(sd);
    sd.flags = s.flags;
    ...
    subpassdeps.emplace_back();
    VkOverwrite(subpassdeps.back());
    subpassdeps.back().srcSubpass = dep2.srcSubpass;
    subpassdeps.back().dstSubpass = dep2.dstSubpass;
    subpassdeps.back().srcStageMask = dep2.srcStageMask;
    subpassdeps.back().dstStageMask = dep2.dstStageMask;
    subpassdeps.back().dependencyFlags = dep2.dependencyFlags;
  }
