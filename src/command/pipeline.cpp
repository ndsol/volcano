...
VkImageAspectFlags PipelineAttachment::aspectMaskFromFormat(VkFormat format) {
  VkImageAspectFlags r =
      (FormatIsColor(format) ? VK_IMAGE_ASPECT_COLOR_BIT : 0) |
      (FormatHasDepth(format) ? VK_IMAGE_ASPECT_DEPTH_BIT : 0) |
      (FormatHasStencil(format) ? VK_IMAGE_ASPECT_STENCIL_BIT : 0) |
  ...
  // FIXME: VK_IMAGE_ASPECT_METADATA_BIT is not handled.
  return r;
}
