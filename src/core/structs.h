...
// Special cases GRAPHICS and COMPUTE request a VkQueue with
// (queueFlags & VK_QUEUE_GRAPHICS_BIT) or (queueFlags & VK_QUEUE_COMPUTE_BIT)
// in Instance::requestQfams() and Device::getQfamI().
//
...
enum SurfaceSupport {
  ...
  COMPUTE = 0x1001,   // Special case. Not used in QueueFamilyProperties.
};
...
struct QueueFamilyProperties : VkQueueFamilyProperties2 {
  ...
  inline bool isCompute() const {
    return queueFamilyProperties.queueFlags & VK_QUEUE_COMPUTE_BIT;
  }
...
// SurfaceCapabilities gathers all the structures that are supported by
// Volcano for VkSurfaceCapabilities2KHR.
struct SurfaceCapabilities : VkSurfaceCapabilities2KHR {
  SurfaceCapabilities() { reset(); }
  SurfaceCapabilities(const VkSurfaceCapabilities2KHR& c)
      : VkSurfaceCapabilities2KHR(c) {}

  // reset clears all structures to their default (zero) values.
  void reset();

  // getProperties is a wrapper for vkGetPhysicalDeviceSurfaceCapabilitiesKHR
  // (and VkPhysicalDeviceSurfaceInfo2KHR).
  //
  // This form takes as input the VkSurfaceKHR.
  WARN_UNUSED_RESULT VkResult getProperties(Device& dev, VkSurfaceKHR s);

  VkDisplayNativeHdrSurfaceCapabilitiesAMD nativeHdr;
  VkSharedPresentSurfaceCapabilitiesKHR sharedPresent;
#ifdef VK_EXT_FULL_SCREEN_EXCLUSIVE_EXTENSION_NAME
  // monitor is used to query fullscreenExclusive support.
  // NOTE: NVidia and AMD appear to have shipped fullscreenExclusive without
  // implementing vkGetPhysicalDeviceSurfaceCapabilities2KHR. In other words,
  // fullscreenExclusive is set by attempting
  // vkGetPhysicalDeviceSurfacePresentModes2EXT with
  // VkSurfaceFullScreenExclusiveInfoEXT. If it succeeds, fullscreenExclusive
  // is supported.
  HMONITOR monitor;
  VkSurfaceCapabilitiesFullScreenExclusiveEXT fullscreenExclusive;
#endif /*VK_EXT_FULL_SCREEN_EXCLUSIVE_EXTENSION_NAME*/
  VkSurfaceProtectedCapabilitiesKHR drm;
};
...
// NOTE: if an error occurs, returns "failed:" + some error string.
