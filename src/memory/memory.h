...
typedef struct VmaAllocation {
  ...
  VmaAllocation(VmaAllocation&& other) = default;
...
typedef struct DeviceMemory {
  ...
  DeviceMemory(DeviceMemory&& other)
    ...
    if (other.lockmutex.try_lock()) {
      ...
      memcpy(&allocInfo, &other.allocInfo, sizeof(allocInfo));
      other.allocInfo.pMappedData = nullptr;
    } else {
  ...
  // allocSize returns the size in bytes of this memory allocation if alloc()
  // has been called. allocSize returns 0 if alloc() has not been called yet.
  //
  // For a Buffer, allocSize() returns Buffer::info.size.
  // For an Image, allocSize() returns a value computed from MemoryRequirements
#ifdef VOLCANO_DISABLE_VULKANMEMORYALLOCATOR
  size_t allocSize() const { return vmaAlloc.allocSize; }
#else /*VOLCANO_DISABLE_VULKANMEMORYALLOCATOR*/
  size_t allocSize() const { return allocInfo.size; }
#endif
...
typedef struct Image {
  ...
  // science::Sampler::ctorError(), automatically sets up an image for a shader
...
typedef struct MemoryRequirements {
  ...
  void reset() {
    ...
    dedicated.sType = autoSType(dedicated);
    memset(&vkalloc, 0, sizeof(vkalloc));
    vkalloc.sType = autoSType(vkalloc);
    vk.pNext = &dedicated;
#else /*VOLCANO_DISABLE_VULKANMEMORYALLOCATOR*/
