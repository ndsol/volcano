...
int DeviceMemory::alloc(MemoryRequirements req) {
  ...
  if (!dev.vmaAllocator) {
    ...
    if (!vkGetPhysicalDeviceProperties) {
      logF("please call glfwAndroidMain(). It calls InitVulkan().\n");
    }
  ...
  vmaGetAllocationInfo(dev.vmaAllocator, vmaAlloc, &allocInfo);
  return 0;
}
...
int DeviceMemory::flush() {
  VkResult v =
      vmaFlushAllocation(dev.vmaAllocator, vmaAlloc, 0, allocInfo.size);
  if (v != VK_SUCCESS) {
...
int DeviceMemory::invalidate() {
  VkResult v =
      vmaInvalidateAllocation(dev.vmaAllocator, vmaAlloc, 0, allocInfo.size);
  if (v != VK_SUCCESS) {
