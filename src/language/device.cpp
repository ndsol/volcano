...
int Device::initSurfaceFormatAndPresentMode() {
  if (Vk::getSurfaceFormats(phys, getSurface(), surfaceFormats) ||
      Vk::getPresentModes(phys, getSurface(), presentModes)) {
    return 1;
  }
...
Device::Device(language::Instance& inst, VkPhysicalDevice phys)
  ...
  swapChainInfo.sType = autoSType(swapChainInfo);
  ...
  lockmutex = std::make_shared<std::recursive_mutex>();
#endif
}
...
int Instance::createDevices(std::vector<VkPhysicalDevice>& physDevs) {
  ...
  for (const auto& phys : physDevs) {
    ...
    memset(&physProp, 0, sizeof(physProp));
