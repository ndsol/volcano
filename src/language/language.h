...
class VkDebugPtr : public VkPtr<T> {
 public:
  ...
  int setName(const std::string& name) {
    ...
    if (name.empty() && !isNameSet) {
      return 0;  // Do not set name if it is just empty.
    }
    isNameSet = true;
  ...
  int onCreate() {
    ...
    if (VkPtr<T>::object && (isNameSet || !name.empty())) {
      isNameSet = true;
      return setObjectName(dev, VOLCANO_CAST_UINTPTR(VkPtr<T>::object),
                           getObjectType(VkPtr<T>::object), name.c_str());
    }
 ...
 protected:
  ...
  std::string name;
  // isNameSet is a workaround: the Vulkan spec says setting the empty string
  // unsets it. But renderDoc says "<empty>" vs. "Baked Command Buffer NNN" -
  // a worse experience if the empty string is used. So avoid calling
  // setObjectName() unless the name is not empty, or the name was set before.
...
typedef struct Device {
  ...
  std::shared_ptr<std::recursive_mutex> lockmutex;
#endif
  ...
  // getSurface returns VK_NULL_HANDLE until Instance::surface is created.
  // getSurface then returns Instance::surface. This surface is created in
  // Instance::ctorError.
  VkSurfaceKHR getSurface();

  // destroySurface handles an Android-specific case where the OS sends a
  // notification and the app must immediately destroy the surface handle. The
  // app then hangs around in a zombie-like state to see whether the OS will
  // kill it or it can recreate another surface.
  void destroySurface();

  // scap is populated when you call resetSwapChain().
  SurfaceCapabilities scap;
  ...
  WARN_UNUSED_RESULT int setName(const std::string& name) {
    ...
    return setObjectName(*this, (uintptr_t)dev, getObjectType((VkDevice)dev),
                         name.c_str());
  }
