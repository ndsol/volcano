...
int Instance::initQueues(std::vector<QueueRequest>& request) {
  ...
  if (request.size() > 1) {
    // Typically Vulkan 1.0 hits this, because it does not have deviceUUID to
    // detect duplicate devices in language.cpp Instance::Instance().
    logE("Error: which device do I really want? (%zu devs)\n", request.size());
    logE(
        "See "
    ...
    for (size_t dev_i = 0; dev_i < devs.size(); dev_i++) {
      logE("  dev[%zu]: \"%s\"\n", dev_i,
           devs.at(dev_i)->physProp.properties.deviceName);
    }
    return 1;
  }
  return 0;
