...
int loadGLIimage(const char* imgFilenameFound, sk_sp<SkData> data,
  ...
  // image.info.format can be set directly from tex.format() unless it is
  // a format not also supported by OpenGL.
  if (tex.format() > gli::FORMAT_RGBA_ASTC_12X12_SRGB_BLOCK16) {
    logE("skiaglue::loadImage: format %u needs a custom conversion to vulkan\n",
  ...
  logE(
      "calculating readOfs should be done by reading gli code and doing what "
      "is done there. compressed formats will break!\n");
  uint32_t formatBytes = gli::block_size(tex.format());
...
int loadSkiaImage(const char* imgFilenameFound, sk_sp<SkData> data,
  ...
  if (img.info.format > VK_FORMAT_ASTC_12x12_SRGB_BLOCK) {
    logE("skiaglue::loadImage: format %u is not supported by gli.\n",
         img.info.format);
    return 1;
  }
  uint32_t rowStride =
      dim.width() * gli::block_size(static_cast<gli::format>(img.info.format));
...
static int writeMmapped(VkExtent3D extent, gli::format format,
  ...
  if (format > gli::FORMAT_RGBA_ASTC_12X12_SRGB_BLOCK16) {
    logE("skiaglue::writeToFile(%s): format %u needs a custom converter\n",
         outFilename, static_cast<unsigned>(format));
    return 1;
  }
  ...
  if (fileType == FT_UNKNOWN) {
    ...
    // Only mip level 0 of mappedMem is used.
    SkPixmap src(SkImageInfo::Make(extent.width, extent.height,
                                   kRGBA_8888_SkColorType, kPremul_SkAlphaType),
                 mappedMem, gli::block_size(format) * extent.width);
...
int skiaglue::writeToFile(memory::Image& image, memory::Stage& stage,
                          ...
                          const std::vector<VkBufferImageCopy>& copies) {
  if (image.info.format > VK_FORMAT_ASTC_12x12_SRGB_BLOCK) {
    logE("skiaglue::writeToFile(%s): format %u needs a custom converter\n",
         outFilename.c_str(), image.info.format);
    return 1;
  }
  VkDeviceSize offset = 0;
  VkDeviceSize formatSize =
      gli::block_size(static_cast<gli::format>(image.info.format));
