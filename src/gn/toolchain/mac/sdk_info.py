...
def FillXcodeVersion(settings):
  ...
  # if xcrun does not have any SDKs installed, fail now.
  sdk_path_xrun_default = CheckOutput([
      'xcrun', '--show-sdk-path']).strip()
  # if xcrun only has CommandLineTools SDKs installed, do not run xcodebuild
  if '/CommandLineTools/' in sdk_path_xrun_default:
    return
...
def FillSDKPathAndVersion(settings, platform):
  ...
  if 'xcode_version' in settings:
    have_build_ver = settings['xcode_version'] >= '0720'
...
if __name__ == '__main__':
  ...
  FillSDKPathAndVersion(settings, unknownargs[0])
