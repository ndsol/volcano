...
def main():
  ...
  out, err_unused = job.communicate()
  if out is not None:
  ...
  sdk_paths = [
    ...
    'SDKs', # for CommandLineTools
  ]
  sdk_dir = None
  for subdir in sdk_paths:
    subdir = os.path.join(out, subdir)
    if os.path.isdir(subdir):
      sdk_dir = subdir
      break
  if sdk_dir is None:
    raise Exception('xcode-select -print-path: "%s" but no "SDKs" dir found' % out)
  ...
  if options.print_sdk_path:
    xcrun = subprocess.check_output(
        ['xcrun', '-sdk', 'macosx' + best_sdk, '--show-sdk-path'])
    print(xcrun.decode(sys.stdout.encoding).strip())
